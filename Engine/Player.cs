...
namespace Engine
{
    public class Player : LivingCreature
    {
        ...
        {
            // See if the player has all the items neede to complete the quest here
            foreach(QuestCompletionItem qci in quest.QuestCompletionItems)
            {
                bool foundItemInPlayersInventory = false;

                // Check each item i nthe player's inventory, to see if they have it, and enough of it
                foreach(InventoryItem ii in Inventory)
                {
        ...
        public void AddItemToInventory(Item itemToAdd)
        {
            foreach(InventoryItem ii in Inventory)
            {
                ...
                {
                    // The have the item i ntheir inventory, so increase the quantity by one
                    ii.Quantity++;
        ...
        public void MarkQuestCompleted(Quest quest)
        {
            ...
            foreach(PlayerQuest pq in Quests)
            {
                if(pq.Details.ID == quest.ID)
                {
                    ...
                    return; // We found the quest, and mark it as completed, so get out of this function
                }
            }
        }
    }
